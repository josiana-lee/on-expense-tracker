...
## 4-5. 할부 — 파생이 아니라 회차별 행으로 저장한다

할부는 진실이 둘이다. "120만원을 샀다"(지출)와 "이번 달 40만원이 빠진다"(현금흐름)가 둘 다 맞다. 이 앱은 **현금흐름 쪽을 저장**한다. 3개월 할부 120만원은 40만원짜리 행 세 개가 되어 구매일, 그 다음 달, 그 다음다음 달에 각각 놓인다.

**한 행에 개월 수만 달고 읽는 쪽에서 나누는 방법을 쓰지 않은 이유.** 이 앱의 조회는 전부 "그 날짜 범위의 행을 더한다"로 되어 있다 — 달력 일별 합계, 월 합계, 예산 소진, 카드 청구액, 검색, 통계. 파생 방식이면 그 여섯 곳이 모두 할부를 알아야 하고, 한 곳만 빠뜨려도 숫자가 조용히 틀린다. 게다가 이번 달 달력이 지난 11개월치 행까지 끌어와야 해서 4-1의 핫 경로가 무거워진다. 행으로 만들어두면 읽는 쪽은 손댈 것이 없다.

**대가는 쓰는 쪽이 치른다.** 회차는 묶음으로만 다룬다.

- 금액은 회차별로 수정할 수 없다. 하나만 고치면 회차 합이 결제 총액과 어긋나는데, 그 상태를 화면에 설명할 방법이 없다.
- 카테고리·결제수단·메모·세부항목 수정은 묶음 전체에 적용한다. 회차마다 카테고리가 다른 할부는 읽을 수 없다.
- 삭제도 묶음 전체다. 회차 하나만 사라진 할부는 사용자가 복구할 방법이 없다.
- **수정과 삭제는 생성과 마찬가지로 한 트랜잭션 안에서 한다.** 회차별로 나눠 커밋하면 도중에 끊겼을 때(툼스톤은 지운 행 전체를 복사하므로 저장공간이 빠듯한 기기에서 실패가 나는 지점이 여기다) 카테고리가 섞이거나 반쪽만 남은 묶음이 생기고, 그 상태는 화면에 설명할 방법이 없다. 커밋이 한 번이면 liveQuery 무효화도 한 번이라, 24개월 할부를 고칠 때 달력이 24번 다시 그려지지도 않는다.

**필드는 전부 optional·비인덱스다** (`installmentId` / `installmentNo` / `installmentMonths` / `installmentTotal`). 9-2의 원칙대로 Dexie 버전업이 필요 없고, 따라서 이미 설치된 기기의 DB를 건드리지 않는다. 묶음 조회는 인덱스 없는 전체 스캔이지만, 화면을 그리는 경로가 아니라 사용자가 한 건을 열거나 지울 때만 돈다.

**나머지 원은 첫 회차에 얹는다.** 100만원 3개월은 333,334 / 333,333 / 333,333이다. 카드사 관례이기도 하고, 마지막 회차에 얹으면 몇 달 뒤에야 어긋난 금액이 나타나서 사용자가 앱을 의심한다. 회차 금액의 합이 결제 총액과 같다는 것이 이 기능의 유일한 불변식이고, `installments.test.ts`가 1원부터 400원까지 모든 조합으로 검사한다.

**회차 날짜는 늘 구매일에서 다시 센다.** 직전 회차 날짜에 한 달씩 더하면 1/31이 2/28로 당겨진 뒤 3/28, 4/28로 굳어 하루씩 잃는다. 없는 날짜는 그 달 말일로 당긴다.

**개월 수는 칩이 아니라 직접 입력이다.** 카드사마다 거는 개월 수가 제각각이라(7·10·18·24개월 특별 할부) 몇 개를 골라 칩으로 내놔도 없는 것을 쓰는 사람이 반드시 나온다. 입력 자릿수 두 자리를 상한으로 삼는다.

**복원 스키마에는 개월 수 상한을 두지 않는다.** 앱이 받는 상한이 나중에 늘어나면, 상한이 박힌 구버전 앱에서 그 백업을 복원할 때 해당 행이 통째로 버려진다 — 검증이 돈을 지우는 셈이다. 정수·양수만 확인한다.

**칩은 채우지 않고 테두리로 상태를 말한다.** 결제수단 칩은 선택되면 브랜드색으로 꽉 차는데, 바로 아래 줄까지 같은 방식이면 두 줄이 같은 무게로 경쟁해서 어느 쪽이 무엇인지 한눈에 안 갈린다. 채움은 결제수단, 테두리는 할부 — 색이 같으니 둘이 한 묶음이라는 건 그대로 읽힌다.
...
**회차 분배는 개월 수를 고르는 시트에서만 말한다.** 고르고 나온 사람에게 같은 계산을 메인 화면에서 한 번 더 읽히는 건 참견이다. 칩에 적힌 "3개월 할부"면 무엇을 골랐는지는 충분하다.

**할부 선택은 결제수단 칩 바로 아래에 둔다.** 처음에는 금액 시트 안에 넣었는데, 사용자는 금액을 먼저 넣고 시트를 닫은 뒤에 카드를 고른다. 카드를 누르는 시점에는 시트가 이미 사라져 있어서 할부를 고를 자리가 화면 어디에도 없었다. 할부는 금액이 아니라 결제수단에 딸린 선택이다.
