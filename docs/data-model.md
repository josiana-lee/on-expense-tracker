...
| Pro | **Play Billing 로컬 검증. 서버 영수증 검증 안 함** | 광고 제거 우회의 기대 손실 < 서버 운영비 |
| Pro 저장 위치 | **메인 DB 밖. 백업 대상에서 제외** | 백업 JSON 편집으로 Pro 획득 방지 |
...
1. **신용카드 명세 주기.** 브리프에는 "결제일"만 있다. 한국 신용카드는 보통 `사용기간(전월 X일~당월 Y일) → 결제일 Z일` 구조이고 카드사마다 다르다. 셋 중 하나를 골라야 한다:
   (a) 사용자가 카드 등록 시 사용기간 직접 입력 — 정확하지만 입력 부담
   (b) 결제일로부터 관례적 역산 — 편하지만 부정확
   (c) 주요 카드사 프리셋 제공 — 정확하고 편하지만 유지보수 부담

2. **전월실적 산정 범위.** 카드사마다 실적 제외 항목(세금, 상품권, 할부 등)이 다르다. 단순 합계로 보여줄지, "참고값" 면책을 붙일지.

3. **`accounts.balance` 자동 차감 여부.** 위에서는 자동 차감하지 않는 안을 제안했다.

4. **Pro 상품 형태.** 1회 구매(비소모성)인지 구독인지. 구독이면 갱신 실패·유예 기간 처리가 추가된다.

5. **Pro의 실제 혜택 범위.** "광고 제거"만인지, 클라우드 백업/CSV 등 기능도 포함인지. 서버 기능이 Pro에 들어가면 8-1의 판단이 바뀐다.

6. **Android Auto Backup이 WebView IndexedDB를 실제로 포함하는지.** 7-1 C의 `adb bmgr` 절차로 검증 전까지 백업 수단으로 계산하지 않는다.
...
- [ ] 윤년 2/29, 월말(31일→2월) 반복 지출 clamp
- [ ] 카테고리를 archive해도 과거 기록 표시가 깨지지 않음
- [ ] 카테고리 `type`을 바꿔도 과거 기록의 지출/입금이 안 바뀜

**동시성 / 멱등성**
- [ ] 같은 반복 규칙 materialize를 연속 3회 호출 → 기록 1건만 생성
